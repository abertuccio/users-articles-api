...
test('Correct request but empty token', async () => {
    const response = await request.post('/api/new-user').send({token: "",name: 'john'});    
    expect(response.status).toBe(500);
    expect(typeof response.body).toBe('object');
    expect(response.body.error).toMatch(/Invalid Token format/);
});
...
test('Correct request but invalid token', async () => {
    const response = await request.post('/api/new-user').send({token: "1CD4ED171E1C95FE163B753A297D5",name: 'john'});    
    expect(response.status).toBe(500);
    expect(typeof response.body).toBe('object');
