...
async function articleValidation(User, Article, articleUpdateProposal) {
    ...
    if (!Object.prototype.hasOwnProperty.call(articleUpdateProposal, "title") &&
    ...
    if ("title" in articleUpdateProposal && !/^[\w, ]{3,50}$/.test(articleUpdateProposal.title)) {
        return errMsg(19);
    }

    if ("text" in articleUpdateProposal && !/^[\w,.\r\n ]{3,4000}$/.test(articleUpdateProposal.text)) {
        return errMsg(20);
    }

    if ("tags" in articleUpdateProposal && (!Array.isArray(articleUpdateProposal.tags) || articleUpdateProposal.tags.some(t => !/^[a-z]{3,50}$/.test(t)))) {
        return errMsg(21);
    }
